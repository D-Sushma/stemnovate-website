...
export default async function handle(req, res) {
    try {
        ...
        ejs.renderFile(path.resolve(`./components/email/VerifyEmail.ejs`), { name: name, code: code, url: url }, function (err, data) {
            if (err) {
                console.log(err);
            } else {
                ...
                var mainOptions = {
                    from: from,
                    to: email,
                    subject: "Welcome To Stemnovate!. Verify your email !",
                    html: data,
                };

...
const sliderRef = React.createRef();
  ...
  const { banner_id } = router.query.banner_id
  ...
  useEffect(() => {
    ...
    if (props.promotionDetails.status === 200) {
      ...
      if (banner_id >= 0) {
        const bannerIndex = promotions.ref={sliderRef} {((ele) => ele.id === banner_id)
      ...
      <main id="homepage-one">
        ...
        <div className="ps-top-banners">
          <div className="ps-section--banner ps-banner--container">
            ...
            <Slider ref={sliderRef} {...carouselSetting} className="ps-carousel">
              {promotions &&
...
export async function getServerSideProps() {
  ...
  const res1 = await fetch(
    baseUrl + "/api/homebanner/getAllPreviewbanner",
    requestOptions
  )

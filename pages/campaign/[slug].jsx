...
import { TbChecks, TbPlugOff, TbInfoCircle, TbAccessPointOff } from "react-icons/tb";
...
const CampaignPage = (props) => {
            ...
            try {
                ...
                formValues.forEach((element) => {
                    if (element.formtype == "file") {
                        if (element.file == "" || (element.file == undefined && element.Required)) {
                            error = 1;
                        }
                        formdata.append(element.name, element.file);
                        formdata.append(`fileType[${element.name}]`, element.formtype);
                    } else {
                        if (element.value == "" || (element.value == undefined && element.Required)) {
                            error = 1;
                        }
                ...
                <div>
                    <p className="">
                        ...
                        <Link href={"/auth/UserLogin"}>
                            <u>
                                <a className="text-primary font-weight-bolder">Login</a>
                            </u>
                        </Link>{" "}
                        before submitting. If you don&apos;t have an account. Create one{" "}
                        <Link href={"/auth/UserReg"}>
                            <u>
                                <a className="text-primary font-weight-bolder">here</a>
                            </u>
                        </Link>
                    </p>
                </div>
    ...
    const sendVerifyLink = async (name, email) => {
        ...
        await fetch(process.env.NEXT_BASE_URL + "/api/Email/promotionEmail", requestOptions)
            .then((response) => response.json())
            .then(async (result) => {
                setIsLoading(false);
            ...
            <main className="ps-page ps-page--inner">
                ...
                <div className="ps-page__header  breadcrumb-h application-breadcrumb-bg">
                    <div className="container ">
                        ...
                        <h1 className="text-center  text-white p-2">{productData.data[0].title}</h1>
                    </div>
                </div>
                    ...
                    <div className="ps-about">
                        {productData.data[0].status ? (
                                    ...
                                    <section className="ps-section--block-grid ">
                                        <div className="ps-section__thumbnail">
                                            <a className="ps-section__image" href="#">
                                                <Image src={`${process.env.AWS_S3BUCKET_URL}${productData.data[0].image}`} width={700} height={475} alt={productData.data[0].title} />
                                            </a>
                                        </div>
                                        <div className="ps-section__content">
                                            <div className="ps-section__desc">
                                                <h1>{productData.data[0].campaign_description}</h1>
                                            </div>
                                            <div className="row">
                                                ...
                                                <div className="col-md-4">
                                                    <span className="text-right">{dayjs(productData.data[0].posted_at).format("MMM D, YYYY")}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </section>
                                        ...
                                        status == "success" ? (
                                            <div className="alert alert-success my-5" role="alert">
                                                <h4 className="alert-heading">
                                                    <TbChecks size="25" /> Thank you for your participation
                                                </h4>
                                                <p>Monthly from 1st September 2022, Stemnovate will hold a free photo contest that requires prior purchase from its website. This contest is named `&quot;`Monthly shine through your research`&quot;` and is on the theme of cell culture. Every month, the contest will be available from the first to the 25th of the month.</p>
                                                <hr />
                                                <p className="mb-0">The contest consists of creating a picture concerning cell culturing using Stemnovate`&apos;`s products. The winners will be featured on our social media and website and will win a small prize.</p>
                                            </div>
                                        ) : status == "error" ? (
                                            ...
                                            <div className="alert alert-info my-5" role="alert">
                                                <h4 className="alert-heading">
                                                    <TbInfoCircle size="25" /> Something details are missing!
                                                </h4>
                                                <p>Monthly from 1st September 2022, Stemnovate will hold a free photo contest that requires prior purchase from its website. This contest is named `&quot;`Monthly shine through your research`&quot;` and is on the theme of cell culture. Every month, the contest will be available from the first to the 25th of the month.</p>
                                                <hr />
                                                <p className="mb-0">The contest consists of creating a picture concerning cell culturing using Stemnovate`&apos;`s products. The winners will be featured on our social media and website and will win a small prize.</p>
                                                    ...
                                                    <form onSubmit={SubmitForm}>
                                                        <div className="row">
                                                            {formValues.map((val, key) => (
                                                                <div key={key} className="col-md-6 col-sm-6 col-xs-1 form-group">
                                                                    {val.formtype == "textarea" ? (
                                                                        <div className="">
                                                                            <label htmlFor="inputPassword5">{val.name.replace(/_/g, " ")}</label>
                                                                            <textarea name={val.name} placeholder={val.helpingText} onChange={(e) => handleChange(key, e)} required={val.Required} className="form-control">
                                                                                {val.value}
                                                                            </textarea>
                                                                        </div>
                                                                    ) : val.formtype == "checkbox" ? (
                                                                        <div className="form-check">
                                                                            <input className="form-check-input form-control" type="checkbox" required={val.Required} name={val.name} onChange={(e) => handleChange(key, e, true)} id={"checkbox_" + key} />
                                                                            <label className="form-check-label" htmlFor={"checkbox_" + key}>
                                                                                <a href="/campaign-terms-and-condition" className="btn btn-link btn-lg" target={"_blank"}>
                                                                                    <u> {val.helpingText} </u>
                                                                                </a>
                                                                            </label>
                                                                        </div>
                                                                        ...
                                                                        <div className="">
                                                                            <label htmlFor="inputPassword5">{val.name.replace(/_/g, " ")}</label>
                                                                            <input type={val.formtype} name={val.name} placeholder={val.helpingText} value={val.value} onChange={(e) => handleChange(key, e)} required={val.formtype == "file" ? false : val.Required} accept={val.formtype == "file" ? val.filetype : "/*"} className="form-control" />
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            ))}
                                                            {uploadFiles.map((file, index) => {
                                                                return (
                                                                    <div key={index} className="col-md-12 col-sm-12 col-xs-12 form-group border">
                                                                        <Image src={file} width="150px" height={"150px"} alt="Product Image" />
                                                                    </div>
                                                                );
                                                            })}
                            ...
                            <div className="container">
                                <div className="alert alert-info my-5" role="alert">
                                    <h4 className="alert-heading">
                                        <TbAccessPointOff size="25" /> This campaign currently not active
                                    </h4>
                                    <p>This Campaign is now offline </p>
                                    <hr />
                                    <p className="mb-0">For more information. please contact our support Team </p>
                                </div>
                            </div>
                        )}
...
export async function getServerSideProps({ query }) {
    ...
    if (slug != undefined) {
        ...
        const res = await fetch(baseUrl + "/api/campaign/getallcampaign", requestOptions);
